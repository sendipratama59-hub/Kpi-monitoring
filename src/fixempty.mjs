...
const emptyBlock = `       if (totalItems === 0) {
...
const newEmptyBlock = `       if (totalItems === 0) {
          if (activeTab === 'sudah') {
...
data = data.replace(emptyBlock.replace(/\\`/g, '\`'), newEmptyBlock.replace(/\\`/g, '\`'));
fs.writeFileSync(path, data);
console.log('done block 2');
