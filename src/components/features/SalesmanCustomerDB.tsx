...
import { Users, Search, Loader2, UploadCloud, FileSpreadsheet, Trash2, Trash, AlertCircle, Plus } from 'lucide-react';
...
export function SalesmanCustomerDB() {
  const { showAlert } = useAlert();
  const [data, setData] = useState<SalesmanCustomer[]>([]);
  ...
  // Manual Form State
  const [newSalesmanCode, setNewSalesmanCode] = useState('');
  ...
  const fetchData = async () => {
    ...
    setCurrentPage(1);
    try {
      let allRecords: SalesmanCustomer[] = [];
      let from = 0;
      const limit = 1000;
      let hasMore = true;
      ...
      while (hasMore) {
        ...
        if (records && records.length > 0) {
          allRecords = [...allRecords, ...records as SalesmanCustomer[]];
          from += limit;
    ...
    try {
      ...
      const mappedData = parsedData.map(row => {
        ...
        return {
          ...
          salesman_code: getVal(['salesman_code', 'kode_salesman', 'kode_sales', 'kode sales', 'kodesales']),
          salesman_name: getVal(['salesman_name', 'nama_salesman', 'nama_sales', 'nama sales', 'namasales']),
          customer_code: getVal(['customer_code', 'kode_customer', 'kode_cust', 'kode cust', 'kodecustomer']),
          customer_name: getVal(['customer_name', 'nama_customer', 'nama_cust', 'nama cust', 'namacustomer']),
        };
      }).filter(item => item.salesman_code || item.customer_code);

      if (mappedData.length === 0) {
        throw new Error("Tidak dapat menemukan kolom yang sesuai (salesman_code, salesman_name, dll)");
      }
  ...
  const handleSaveToDatabase = async () => {
    ...
    setError(null);
    try {
      // Remove temporary ID before inserting
      const dataToInsert = previewData.map(({ id, ...rest }) => rest);
      
      const BATCH_SIZE = 500;
      for (let i = 0; i < dataToInsert.length; i += BATCH_SIZE) {
        const batch = dataToInsert.slice(i, i + BATCH_SIZE);
        const { error: insertError } = await supabase.from('salesman_customer').insert(batch);
  ...
  const handleSaveManual = async () => {
    if (!newSalesmanCode || !newSalesmanName) {
      showAlert('Kode Salesman dan Nama Salesman harus diisi.', 'warning');
      return;
    }
    ...
    try {
      const { error: insertError } = await supabase
        .from('salesman_customer')
        .insert([{
          salesman_code: newSalesmanCode,
          salesman_name: newSalesmanName,
      ...
      console.error(err);
      showAlert(err.message || 'Gagal menambahkan data secara manual.', 'error');
    } finally {
  ...
  const handleDeleteAll = async () => {
    setLoading(true);
    try {
      const { error } = await supabase.from('salesman_customer').delete().neq('id', '00000000-0000-0000-0000-000000000000'); // Delete all trick
  ...
  const filteredData = (previewData || data).filter(d => {
    const namaSalesman = String(d.salesman_name || '').toLowerCase();
    const namaCustomer = String(d.customer_name || '').toLowerCase();
    const kodeSalesman = String(d.salesman_code || '').toLowerCase();
    const kodeCustomer = String(d.customer_code || '').toLowerCase();
    const searchLower = searchTerm.toLowerCase();
    return namaSalesman.includes(searchLower) || 
           namaCustomer.includes(searchLower) ||
           kodeSalesman.includes(searchLower) ||
           kodeCustomer.includes(searchLower);
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 mb-8">
        <h2 className="text-3xl font-black tracking-tight text-slate-800 flex items-center gap-3">
          ...
          Database Salesman & Customer
        </h2>
        <p className="text-slate-500 font-medium">Kelola master data relasi salesman dan customer</p>
      </div>
          ...
          <Card className="border-none shadow-xl shadow-slate-200/60 overflow-hidden">
            ...
            <CardHeader className="pt-6">
              ...
              <CardDescription className="text-xs font-medium">
                Upload file format .xlsx dengan kolom: salesman_code, salesman_name, customer_code, customer_name
              </CardDescription>
            </CardHeader>
        ...
        <div className="lg:col-span-2">
          <Card className="h-full flex flex-col border-none shadow-xl shadow-slate-200/60 overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                  ...
                  <div className="relative flex-1 md:w-64">
                    ...
                    <input 
                      type="text" 
                      placeholder="Cari salesman atau customer..." 
                      value={searchTerm}
                      onChange={e => {
            ...
            <ConfirmModal
              ...
              message="Yakin ingin menghapus seluruh data Salesman & Customer? Tindakan ini tidak dapat dibatalkan."
              isDestructive={true}
              ...
              <div className="absolute inset-0 overflow-auto">
                <table className="w-full text-sm text-left whitespace-nowrap">
                  <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b sticky top-0 z-10">
                    <tr>
                      <th className="px-4 py-3">Kode Salesman</th>
                    ...
                    {loading ? (
                      ...
                      paginatedData.map((row) => (
                        <tr key={row.id} className="border-b hover:bg-slate-50">
                          <td className="px-4 py-3 font-medium text-slate-700">{row.salesman_code}</td>
                          <td className="px-4 py-3 text-slate-900">{row.salesman_name}</td>
                          <td className="px-4 py-3 font-medium text-slate-700">{row.customer_code}</td>
      ...
      <GenieModal
        ...
        title="Tambah Data Manual"
        maxWidth="max-w-md"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Kode Salesman</label>
              <input
                type="text"
                value={newSalesmanCode}
                onChange={e => setNewSalesmanCode(e.target.value)}
                placeholder="Contoh: S001"
                className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Nama Salesman</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Kode Customer (Opsional)</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Nama Customer (Opsional)</label>
              <input
                type="text"
